...
def test_chat_partial_update_true(db, client, marian_auth_token, chat_marian_egor):
    data = {"is_done": True, "is_approved": True}
...
def test_chat_partial_update_false(db, client, egor_auth_token, chat_egor_marian):
    data = {"is_done": True, "is_approved": True}
    response = client.patch(
    ...
    assert response.status_code == 204
    data = {"is_done": False, "is_approved": False}
    response = client.patch(
        f"chats/{chat_egor_marian}",
        json=data,
        headers=egor_auth_token,
    ...
    assert response.status_code == 204
    db.execute(
        f"SELECT is_done, is_approved FROM chats WHERE chat_id ='{chat_egor_marian}'"
    )
    chat = db.fetchone()
...
def test_chat_partial_update_is_approved_true(
        db,
        client,
        marian_auth_token,
        chat_marian_egor
):
    data = {"is_done": False, "is_approved": True}
    response = client.patch(
        f"chats/{chat_marian_egor}",
        json=data,
        headers=marian_auth_token,
...
def test_chat_partial_update_is_approved_false(
        db,
        client,
        marian_auth_token,
        chat_marian_egor
