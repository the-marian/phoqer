...
export const InProgressOrders = (): JSX.Element => {
    ...
    useEffect(() => {
        ordersService
            .getOrders({ status: OrderStatus.IN_PROGRESS, limit: isTablet ? UiPagination.LX : UiPagination.SX })
            .then(setOrders)
            .catch(errorToast)
            .finally(() => setIsLoading(false));
    }, [errorToast]);

    return <OrdersPreview isLoading={isLoading} orders={orders} title={t('Your active orders')} />;
};
